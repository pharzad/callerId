...
ShareData sd = new ShareData();
   ...
   static ImageView im;
        ...
        public void onCallStateChanged(int state, String incomingNumber)
        {
                switch (state){
                    case TelephonyManager.CALL_STATE_RINGING:
                        int currentCountryCompare = sd.getData().compareToIgnoreCase("USA");
                        Toast.makeText(getApplicationContext(),incomingNumber,Toast.LENGTH_LONG).show();
                        if (incomingNumber.length()<=10) {
                            String firstThree = incomingNumber.substring(0,3);
                            try {

                                    JSONObject usaAreaCode = new JSONObject(usaAreaCodeString);
                                    Iterator keys = usaAreaCode.keys();
                                    while (keys.hasNext()) {
                                        String key = (String) keys.next();
                                        JSONArray ar = usaAreaCode.getJSONArray(key);
                                        for (int i = 0; i < ar.length(); i++) {
                                            int compare = ar.getString(i).compareToIgnoreCase(firstThree);
                                            if (compare == 0) {
                                                if(currentCountryCompare==0) {
                                ...
                                //Possible Country
                                String areaCode1 = incomingNumber.substring(0,2);
                                String areaCode2 = incomingNumber.substring(0,3);

                                try {
                                    JSONArray codes = new JSONArray(counteryCodes);
                                    for (int i = 0 ; i<codes.length(); i++)
                                    {
                                        JSONObject temp = codes.getJSONObject(i);
                                        int compare = temp.getString("dial_code").substring(1).compareTo(areaCode1);
                                        if(compare == 0)
                                        {
                                            Toast.makeText(getApplicationContext(),"Possible Country:"+temp.getString("name"),Toast.LENGTH_LONG).show();
                                        }
                                        compare = temp.getString("dial_code").substring(1).compareTo(areaCode2);
                                        if(compare == 0)
                                        {
                                            Toast.makeText(getApplicationContext(),"Possible Country:"+temp.getString("name"),Toast.LENGTH_LONG).show();
                                        }
                        ...
                        {
                            int tempLeng = incomingNumber.length() - 10;
                            String areaCode = incomingNumber.substring(0,tempLeng);
                            try {
                                JSONArray codes = new JSONArray(counteryCodes);
                                    for (int i = 0 ; i<codes.length(); i++)
                                    {
                                         JSONObject temp = codes.getJSONObject(i);
                                        int compare = temp.getString("dial_code").substring(1).compareTo(areaCode);
                                        if(compare == 0)
                                        {
                                            Toast.makeText(getApplicationContext(),temp.getString("name"),Toast.LENGTH_LONG).show();
                                        }
                                    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tracker = analytics.newTracker("UA-61764671-2");
        tracker.enableExceptionReporting(true);
        tracker.enableAdvertisingIdCollection(true);
        tracker.enableAutoActivityTracking(true);
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_main);
        RadioGroup rg = (RadioGroup) MainActivity.this.findViewById(R.id.country_group);
        ...
        LinearLayout bg = (LinearLayout) findViewById(R.id.contentPanel);
        Bitmap image = BitmapFactory.decodeFile(getResources().getResourceName(R.drawable.logo));
        im = (ImageView)findViewById(R.id.portOfSolutionsImage);
        im.setImageResource(R.drawable.logo);
        Handler myHandler = new Handler();
        myHandler.postDelayed(runAble, 3000);
    }
